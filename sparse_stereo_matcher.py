...
class SparseStereoMatcher:
    ...
    def _extract_good_matches(self, matches):
        ...
        for match in matches:  # fixme enklere måte?
            epipolar_is_ok = np.abs(self._keypoints_left[match.queryIdx].pt[0] - self._keypoints_right[match.trainIdx].pt[0]) < 1.0
            disparity_is_positive = self._keypoints_left[match.queryIdx].pt[1] > self._keypoints_right[match.trainIdx].pt[1]
